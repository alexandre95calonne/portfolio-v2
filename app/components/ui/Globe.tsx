...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  useEffect(() => {
    if (globeRef.current) {
      const positions = (globeRef.current as any).geometry?.attributes?.position
        ?.array;
      if (positions && positions.some(isNaN)) {
        console.warn("Invalid positions detected");
        return;
      }
      (globeRef.current as any).geometry?.computeBoundingSphere();
    }
  }, []);
